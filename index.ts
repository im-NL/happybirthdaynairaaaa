...
const music = new Audio('happybirthday.mp3')
        ...
        if(guesses<7) {
            if (valid) {
                ...
                if(correct_count == 5) {                    
                    music.play()
                    music.loop=true
                    document.getElementById("word").innerHTML = "SLAYYYYYYY WOOOOHOOOOOOOO"
                    document.getElementById("winpopup").classList.remove("win")
                    document.body.setAttribute("style", "background-image:url('bg.png')")
                    startConfetti();
                    document.getElementById("winpopup").classList.add("visible-win")
                }
                if(correct_count!=5 && guesses===6) {
                    document.getElementById("loseword").innerHTML = ".......don't tell me you just got your name wrong <br> <a href='https://www.instagram.com/notyouraveragegammerkid/'>ok click here ig</a>"
                    document.getElementById("losepopup").classList.remove("lose")
                    document.getElementById("losepopup").classList.add("visible-lose")
                }
                guesses +=1 
